...
export const createShortURL = async (body: {url : string , id? : string } , user_id : number) => {
    ...
    if(body.id){
        ...
        if(current_record){
            throw new httpError.Conflict("The ID that you provided already exists in our database");
        }
    }
...
export const getURLS = async(user_id : number, limit : number, offset : number) => {
    const results = await knex("urls").where({ user_id }).leftJoin("visits" , "urls.id" , "visits.url_id").select(["urls.id" , "urls.url" , "urls.created_at" , knex.raw("count(visits.id) as visits_count"), ]).limit(limit || 15).offset(offset || 0).groupBy("urls_id").orderBy("urls.created_at" , "desc");
    return results;
};
