...
export const getVisitsByURL = async (url_id: string, user_id: number, limit: number, offset: number) => {
    const url = await knex("url").where({ id: url_id }).select(["user_id"]).first();

    if (!url) {
        throw new HttpError("URL not found");
    }

    if (url.user_id !== user_id) {
        throw new HttpError("You don't have permissions to view this URL");
    }
